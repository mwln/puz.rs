use crate::{error::{PuzError, PuzWarning}, types::{Extensions, Rebus}};
...
pub(crate) fn parse_extensions_with_recovery(data: &[u8], width: u8, height: u8) -> Result<(Extensions, Vec<PuzWarning>), PuzError> {
    let mut rebus = None;
                    ...
                    ExtraSection::GRBS => {
                        ...
                        if section_data.len() != expected_size {
                            warnings.push(PuzWarning::SkippedExtension { 
                                ...
                                reason: format!("Size mismatch: expected {} bytes, got {}", expected_size, section_data.len())
                            });
                            continue;
                        }
                        ...
                        match find_section(data, "RTBL") {
                            ...
                            Ok(None) => warnings.push(PuzWarning::SkippedExtension { 
                                section: "GRBS".to_string(), 
                                reason: "RTBL section not found - rebus requires both GRBS and RTBL".to_string()
                            }),
                            Err(e) => warnings.push(PuzWarning::SkippedExtension { 
                                section: "GRBS".to_string(), 
