...
pub enum PuzError {
    ...
    InvalidChecksum { expected: u16, found: u16, context: String },
...
impl fmt::Display for PuzError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            PuzError::InvalidChecksum { expected, found, context } => {
            ...
            PuzError::UnsupportedVersion { version } => {
                write!(f, "Unsupported .puz file version: '{}'. Only standard versions are supported.", version)
            }
...
impl PuzError {
    ...
    pub fn with_context(self, context: &str) -> Self {
        match self {
            ...
            PuzError::ParseError { message, position, context: existing_context } => {
                PuzError::ParseError {
                    message,
                    position,
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            PuzWarning::EncodingIssue { context, recovered } => {
                write!(f, "Encoding issue in {}: {}", context, 
                    if *recovered { "recovered using fallback" } else { "could not recover" })
            }
